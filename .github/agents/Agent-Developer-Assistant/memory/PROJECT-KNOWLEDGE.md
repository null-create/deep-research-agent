...
| `long_term_memory.py` | `AsyncLongTermMemory`: persistent cross-session Neo4j store + `KnowledgeGraph` (GraphRAG). `:Memory` nodes for raw evidence (with `memory_embedding_idx` vector index); `:Entity` nodes, `:RELATES_TO` edges, `:Community` nodes + `:MEMBER_OF` edges for the graph layer. `close()` method for clean driver shutdown. `ltm.graph` exposes `upsert_entity`, `store_relationship`, `find_entities`, `get_relationships`, `get_communities`, `update_communities`, `recall_graph_context`. |
